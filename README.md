...
- **🧠 Dual AI Analysts (Claude + Perplexity):** Integrated agents analyze order flow context and label each move as `real`, `noise`, or `uncertain` — shown as an **AI:** pill on every card, separate from the detector tier.
...
1. Download the Trade Hunter executable (`trade-hunter-*.exe`) from [Gumroad](https://lweiss.gumroad.com/l/trade-hunter)
2. Double-click — browser opens automatically at `http://127.0.0.1:8765`
3. Starts in Simulation mode instantly. No Python, no install.
...
*Both editions start in Simulation mode by default — no API keys needed to try the dashboard.*
...
| Tier | Meaning | Visual |
| :--- | :--- | :--- |
| **watch** | Early — a directional bias is present, but below the notable thresholds. | Standard |
| **notable** | Significant — both the volume and price-move qualifiers are clearly met. | Yellow Edge |
| **signal** | Strongest score tier — volume $\ge 3\times$ and price move $\ge 1.75\times$ the minimum thresholds. | Red Neon Edge |
| **whale-cluster** | Statistical anomaly — coordinated whale entry ($\ge 3$ 99th-percentile trades in 120s, Poisson $p < 0.01$). | Purple Neon Edge |
...
# the app's Markets page and stored in data/watchlist.json — so app updates
...
You don't need to hunt for exact contract IDs. Whether you add markets in the app's **Markets** page or seed them via `KALSHI_MARKETS`, each entry accepts:
...
Manage your watchlist directly from the Dashboard's **Markets** page — add/remove without ever restarting the server. Your list is saved to `data/watchlist.json` and **preserved across app updates**; `KALSHI_MARKETS` in `.env` only seeds it on first run.

---

## 📊 Reading the Dashboard

When an alert fires it is graded into one of the visual tiers described under [**Alert Tiers**](#-alert-tiers) above. Each card also shows the AI analyst's own read as an **AI:** pill (`real` / `noise` / `uncertain`) — a separate judgment from the detector tier.
...
* **Single-Instance Guardian:** The exe automatically terminates any hanging instance on the same port before launching — no zombie processes, no port conflicts.
