...
class SpikeDetector:
    ...
    def process(self, event: MarketEvent, baselines: dict[str, float] | None = None) -> SpikeSignal | None:
        ...
        # Whale clusters honor the same per-market cooldown as regular spikes.
        # Without this, every additional whale trade inside the 120s window
        # emits another signal (Discord alert + analyst LLM call each time).
        if is_whale_cluster and window.last_signal_at is not None:
