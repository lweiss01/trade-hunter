...
def _parse_pid_from_netstat(output: str, port: int) -> int | None:
    """Parse `netstat -ano` output for the PID listening on exactly `port`.

    Matches the local-address port exactly (not a substring) so :443 is not
    confused with :4433, and ignores the foreign address / PID columns.
    """
    for line in output.splitlines():
        if "LISTEN" not in line:
            continue
        parts = line.split()
        # columns: proto, local_addr, foreign_addr, state, pid
        if len(parts) < 2:
            continue
        local_addr = parts[1]  # e.g. "127.0.0.1:8765" or "[::1]:8765"
...
def _get_pid_holding_port(port: int) -> int | None:
    if sys.platform == "win32" or sys.platform == "cygwin" or sys.platform == "msys":
        ...
        try:
            ...
            pid = _parse_pid_from_netstat(output, port)
            if pid is not None:
                return pid
        except Exception:
            pass
