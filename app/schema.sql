...
-- Composite index for the "latest event per market" dashboard query (the /api/state
-- hot path). Without it, that GROUP BY scans the whole events table — ~1.3s on a
-- 450k-row DB. With it, ~0.1s. Applied to existing DBs automatically on next connect.
...
CREATE TABLE IF NOT EXISTS markets (
    ...
    last_yes_price REAL,
    last_volume REAL,
    -- Kalshi hierarchy: the Series or Event tier ticker this market was resolved from.
    -- NULL for markets tracked directly or sourced from non-Kalshi platforms.
    parent_ticker TEXT,
    created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP
);
