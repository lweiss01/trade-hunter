...
class MarketStore:
    ...
    def upsert_event(self, event: MarketEvent) -> None:
        ...
        with self._lock:
            ...
            # Extract Kalshi hierarchy tag from event metadata (set by KalshiPykalshiFeed).
            # None for events from other platforms — COALESCE preserves any existing value.
            parent_ticker = (event.metadata or {}).get("parent_ticker") or None

            # Upsert market metadata
            cursor.execute("""
                INSERT INTO markets (
                    market_id, platform, title, last_event_at, total_events,
                    last_yes_price, last_volume, parent_ticker, updated_at
                )
                VALUES (?, ?, ?, ?, 1, ?, ?, ?, ?)
                ON CONFLICT(market_id) DO UPDATE SET
                    ...
                    parent_ticker = COALESCE(excluded.parent_ticker, parent_ticker),
