...
def analyze_signal(
    ...
    Tries providers in the order suggested by the manager (usually Anthropic first).
    When ``ruleset_store`` is provided, scope-relevant applied rules are injected into
...
class SignalAnalyst:
    ...
    def __init__(
        ...
        self._providers_configured = [
            name for name, enabled in (("anthropic", anthropic_key), ("perplexity", perplexity_key)) if enabled
        ]
...
class TuningGovernor:
    ...
    def review(self, advice: TuningAdvice) -> tuple[bool, str | None]:
        # Fast local check against the active ruleset first — no LLM call needed.
        rule_conflict, rule_reason = self._ruleset_conflicts(advice)
        if rule_conflict:
            return True, rule_reason

        # Fallback: LLM-based conflict check against condensed backlog.
