...
from .config import DATA_ROOT, ASSET_ROOT, record_runtime_status
...
# Additive migrations for the markets table.
# Same pattern as signals: key = column name, value = ALTER TABLE statement.
# Existing databases pick these up automatically on next connect.
_MARKETS_COLUMN_MIGRATIONS: dict[str, str] = {
    "parent_ticker": "ALTER TABLE markets ADD COLUMN parent_ticker TEXT",
}
...
def _quarantine_corrupt_database(path: Path) -> None:
    ...
    for suffix in ("-wal", "-shm"):
        ...
        if sidecar.exists():
            sidecar_quarantine_path = path.with_name(f"{sidecar.name}.corrupt-{stamp}")
            sidecar.replace(sidecar_quarantine_path)
            sidecars.append(suffix)

    if path == DEFAULT_DB_PATH:
        record_runtime_status(
            "database-quarantined",
            "Trade Hunter recreated a clean local database.",
            "The default SQLite database failed integrity checks, was quarantined, and startup continued with a clean database.",
            details={"quarantine_name": quarantine_path.name, "sidecars": sidecars},
            data_root=DATA_ROOT,
        )
...
def _ensure_markets_columns(conn: sqlite3.Connection) -> None:
    """Backfill newer markets columns into existing local databases."""
    existing_columns = {
        row["name"]
        for row in conn.execute("PRAGMA table_info(markets)").fetchall()
    }
    for column, statement in _MARKETS_COLUMN_MIGRATIONS.items():
        if column not in existing_columns:
            conn.execute(statement)
