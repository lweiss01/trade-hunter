...
function renderTuningAdvisor(state) {
  ...
  if (!advisor) {
    ...
    if (analystFailing) {
      ...
      tuningAdvisorEl.innerHTML = `<div class="empty">AI analyst is off — add <code>ANTHROPIC_API_KEY</code> or <code>PERPLEXITY_API_KEY</code> to your <code>.env</code> and restart.</div>`;
    } else {
  ...
  fetch("/api/ruleset", { method: "GET", headers })
    ...
    .then((payload) => {
      ...
      const rows = preview.map((r) => {
        ...
        return '<div style="display:flex;gap:8px;padding:2px 0;font-size:0.82rem;">'
          + '<span class="tuning-tb-id">' + escapeHtml(r.id) + '</span>'
          + '<code style="opacity:.85;">' + escapeHtml(r.text) + '</code>'
          + '<span style="opacity:.6;">' + escapeHtml(scope) + '</span>'
          + '<span style="opacity:.55;">hits:' + escapeHtml(String(r.hits || 0)) + '</span>'
          + '<span style="opacity:.45;">' + escapeHtml(source) + '</span>'
          + '</div>';
      }).join("");
