...
const commercialBadgeEl = document.querySelector("#commercial-badge");
...
const analystAlertEl = document.querySelector("#analyst-alert");
const tbFilterHideAppliedEl = document.querySelector("#tb-filter-hide-applied");
const tbFilterHideRejectedEl = document.querySelector("#tb-filter-hide-rejected");
...
const rulesetPanelEl = document.querySelector("#ruleset-panel");
const rulesetLastRefreshEl = document.querySelector("#ruleset-last-refresh");
const rulesetSummaryEl = document.querySelector("#ruleset-summary");
const rulesetCompactionEl = document.querySelector("#ruleset-compaction");
const rulesetPreviewEl = document.querySelector("#ruleset-preview");
...
const settingAnthropicApiKeyEl = document.querySelector("#setting-anthropic-api-key");
const settingPerplexityApiKeyEl = document.querySelector("#setting-perplexity-api-key");
const settingOpenaiApiKeyEl = document.querySelector("#setting-openai-api-key");
const settingXaiApiKeyEl = document.querySelector("#setting-xai-api-key");
const settingGeminiApiKeyEl = document.querySelector("#setting-gemini-api-key");
const runtimeStatusPanelEl = document.querySelector("#runtime-status-panel");
const runtimeStatusCategoryEl = document.querySelector("#runtime-status-category");
const runtimeStatusBodyEl = document.querySelector("#runtime-status-body");
...
const readOnlySettingsControls = [
  ...
  settingAnthropicApiKeyEl,
  settingPerplexityApiKeyEl,
  settingOpenaiApiKeyEl,
  settingXaiApiKeyEl,
  settingGeminiApiKeyEl,
].filter(Boolean);
...
function getAdminToken() {
  const localToken = adminTokenInputEl ? adminTokenInputEl.value.trim() : "";
  if (localToken) return localToken;
  
  // If we are verified via loopback/backend, we don't strictly need a token in the header,
  // but if we have one in localStorage, we include it just in case.
  return localStorage.getItem("th_admin_token") || "";
}
...
function renderSettings(settingsPayload) {
  ...
  renderRuntimeStatus(settingsPayload?.runtime_status);
  ...
  if (settingEnableKalshiEl) {
    settingEnableKalshiEl.checked = Boolean(settings.enable_kalshi);
    if (!isCommercial) {
      settingEnableKalshiEl.checked = false;
      settingEnableKalshiEl.disabled = true;
      settingEnableKalshiEl.parentElement.title = "Commercial Edition only: Live feeds are disabled in public simulation builds.";
    }
  }
  ...
  if (settingAnthropicApiKeyEl) settingAnthropicApiKeyEl.value = formatPresenceLabel(presence.anthropic_api_key);
  if (settingPerplexityApiKeyEl) settingPerplexityApiKeyEl.value = formatPresenceLabel(presence.perplexity_api_key);
  if (settingOpenaiApiKeyEl) settingOpenaiApiKeyEl.value = formatPresenceLabel(presence.openai_api_key);
  if (settingXaiApiKeyEl) settingXaiApiKeyEl.value = formatPresenceLabel(presence.xai_api_key);
  if (settingGeminiApiKeyEl) settingGeminiApiKeyEl.value = formatPresenceLabel(presence.gemini_api_key);
  ...
  if (!settingsSaveInFlight) {
    if (settingsPayload?.is_admin) {
      setSettingsSaveStatus("Authorized via loopback. Save to persist edits. Restart required to apply.", "ok");
    } else {
      setSettingsSaveStatus("Loaded from .env. Save to persist edits. Restart required to apply.");
    }
  }

  // Update Security description and hide pulse if is_admin
  const statusDescEl = document.getElementById("admin-token-status-desc");
  if (settingsPayload?.is_admin && adminTokenInputEl) {
    adminTokenInputEl.placeholder = "Local Access (No Token Required)";
    adminTokenInputEl.classList.remove("pulse-on-empty");
    if (statusDescEl) {
      statusDescEl.innerHTML = `<span style="color:var(--live);font-weight:600;">✓ Local Access Granted</span><br>You are accessing Trade Hunter from the same machine. Mutations are automatically allowed.`;
    }
  }
}

function renderRuntimeStatus(runtimeStatus) {
  const entry = runtimeStatus?.latest || null;
  if (!runtimeStatusPanelEl || !runtimeStatusBodyEl || !entry) {
    if (runtimeStatusPanelEl) runtimeStatusPanelEl.hidden = true;
    return;
  }

  runtimeStatusPanelEl.hidden = false;
  if (runtimeStatusCategoryEl) {
    runtimeStatusCategoryEl.textContent = entry.category || "diagnostics";
  }

  const support = entry.support_detail
    ? `<details class="runtime-status-detail"><summary>Support detail</summary><pre>${escapeHtml(entry.support_detail)}</pre></details>`
    : "";
  runtimeStatusBodyEl.innerHTML = `
    <div class="settings-label">${escapeHtml(entry.buyer_headline || "Startup status is available.")}</div>
    ${support}
  `;
}
...
function renderAnalystAlert(state) {
  if (!analystAlertEl) return;
  const status = state.analyst_status || {};
  if (status.enabled && status.failing && status.message) {
    analystAlertEl.hidden = false;
    analystAlertEl.innerHTML = `
      <div class="analyst-alert-icon" aria-hidden="true">!</div>
      <div class="analyst-alert-copy">
...
function renderMetrics(state) {
  ...
  const stats = [
    ...
    {
      label: "ai analyst",
      value: config.analyst_enabled
        ? (state.analyst_status?.failing ? "Error" : "On")
        : "Off",
      sub: config.analyst_enabled
        ? (state.analyst_status?.failing
          ? "check credits or API keys"
          : (config.analyst_providers || []).join(" → ") || null)
        : `add API keys to ${config.env_path || ".env"}`,
      tone: config.analyst_enabled
        ? (state.analyst_status?.failing ? "mode-off" : "mode-live")
        : "mode-off",
    },
...
async function applyRecommendedTuning() {
  ...
  try {
    const response = await fetch("/api/config/apply-tuning", { 
      method: "POST",
      headers: {
        "Authorization": `Bearer ${getAdminToken()}`
      }
...
function renderTuningAdvisor(state) {
  ...
  if (!advisor) {
    const analystOff = state.config && state.config.analyst_enabled === false;
    const analystFailing = state.analyst_status && state.analyst_status.failing;
    if (analystFailing) {
      tuningAdvisorEl.innerHTML = `<div class="empty">Tuning recommendations need working analyst readouts. See the warning above.</div>`;
    } else if (analystOff) {
      tuningAdvisorEl.innerHTML = `<div class="empty">AI analyst is off — add <code>ANTHROPIC_API_KEY</code>, <code>GEMINI_API_KEY</code>, or other provider keys to your <code>.env</code> and restart.</div>`;
    } else {
...
function renderSignals(signals) {
  ...
  signalsEl.innerHTML = ordered.map((signal) => {
    ...
    // Tier strings can contain spaces (e.g. "cross-venue divergence"); slugify to
    // a single valid CSS class so the tier pill/card accent get their color.
    const tierCls = "tier-" + tier.trim().toLowerCase().replace(/\s+/g, "-");
    ...
    let analystTagHtml = "";
    if (analyst?.pending) {
      analystTagHtml = `<span class="sig-tag analyst-pending">AI: reviewing</span>`;
    } else if (analyst) {
      ...
      // The analyst's verdict "signal" collides with the detector's "signal" TIER,
      // so relabel it and prefix "AI:" to attribute the whole read to the analyst.
      const nsLabel = ns === "signal" ? "real" : ns;
      const confClass = conf === "high" ? "high-conf" : conf === "medium" ? "med-conf" : "low-conf";
      analystTagHtml = `
        <span class="sig-tag analyst-verdict analyst-${nsClass}">AI: ${escapeHtml(nsLabel)}</span>
        <span class="sig-tag analyst-conf analyst-${confClass}">${escapeHtml(conf)} conf</span>
      `;
    }

    return `
    <article class="signal-card ${tierCls}" data-signal-key="${escapeHtml(getSignalKey(signal))}">
      <div class="sig-row1">
        ...
        <div class="sig-tags">
          ...
          <span class="sig-tag ${tierCls}">${escapeHtml(tier)}</span>
...
function getTrackedTickerRows(tickers, markets, deadTickers = [], hierarchy = {}) {
  ...
  return (tickers || []).map((ticker) => {
    ...
    // Exact market match — the configured ticker IS a tradeable Market contract.
    const exactMatch = (markets || []).find((market) => market?.market_id === ticker) || null;

    // Child market tickers from the backend hierarchy map (authoritative when present).
    // hierarchy[ticker] is an array of resolved child tickers, or undefined if unknown.
    const childTickerList = Object.prototype.hasOwnProperty.call(hierarchy, ticker)
      ? hierarchy[ticker]
      : null;

    let childMatches;
    if (childTickerList !== null) {
      // Authoritative: use the hierarchy map from the backend.
      const childSet = new Set(childTickerList);
      childMatches = (markets || []).filter((market) => childSet.has(market?.market_id));
    } else if (!exactMatch) {
      // Fallback: prefix heuristic for tickers not yet in the hierarchy.
      childMatches = (markets || []).filter(
        (market) => String(market?.market_id || "").startsWith(`${ticker}-`)
      );
    } else {
      childMatches = [exactMatch];
    }

    const bestMatch = exactMatch || childMatches[0] || null;
    const freshness = bestMatch
      ? flowFreshness(bestMatch.timestamp)
      : { label: "unresolved", className: "stale" };
    ...
    let resolvedTitle;
    if (isDeadTicker) {
      resolvedTitle = "Ticker no longer resolves on the Kalshi API";
    } else if (childTickerList !== null) {
      // Hierarchy data available — show authoritative child count.
      const n = childTickerList.length;
      if (n === 0) {
        resolvedTitle = "No active markets (all settled or unreachable)";
      } else if (exactMatch) {
        // Configured ticker is itself a tradeable contract — show its title directly.
        resolvedTitle = bestMatch.title;
      } else {
        // Parent (Series/Event) tier — group child markets under umbrella label.
        const baseTitle = childMatches[0]?.title || ticker;
        resolvedTitle = n === 1 ? baseTitle : `${baseTitle} (${n} markets)`;
      }
    } else if (!bestMatch) {
      resolvedTitle = "No live market snapshot yet";
    } else {
      // Prefix-heuristic fallback path.
      resolvedTitle = exactMatch
        ? bestMatch.title
        : `${bestMatch.title} (${childMatches.length} open markets)`;
    }
...
function renderTickerList(markets) {
  ...
  // kalshi_market_hierarchy: authoritative parent → [child tickers] map from the backend.
  // Provides exact child counts for Series/Event tier tickers (e.g. "KXTRUMPPARDONS (2 markets)").
  const hierarchy = lastDashboardState?.config?.kalshi_market_hierarchy || {};
  const rows = getTrackedTickerRows(trackedTickers, marketSnapshots, deadTickers, hierarchy);
      ...
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          if (result.ok) {
            ...
            // Button state is already updated locally; do NOT re-run the category
            // search (a slow Kalshi network round-trip) — that made the UI hang
            // after every add.
            await refresh();
          } else {
...
async function fetchAndRenderBacklog() {
  ...
  try {
    ...
    const hideApplied = tbFilterHideAppliedEl?.checked ?? false;
    const hideRejected = tbFilterHideRejectedEl?.checked ?? false;
    ...
    const filteredSnapshots = data.snapshots.map(snap => {
      return {
        ...snap,
        items: snap.items.filter(item => {
          if (hideApplied && item.status === "applied") return false;
          if (hideRejected && item.status === "rejected") return false;
          return true;
        })
      };
    }).filter(snap => snap.items.length > 0);

    const snapshotHtml = filteredSnapshots.map(snap => {
...
async function refreshSettings(force = false) {
  ...
  if (typeof renderRulesetPanel === "function") renderRulesetPanel();
  if (typeof renderEvalPanel === "function") renderEvalPanel();
}
...
async function refresh() {
  try {
    ...
    if (upgradeBtnEl) upgradeBtnEl.style.display = isCommercial ? "none" : "flex";
    if (commercialBadgeEl) commercialBadgeEl.style.display = isCommercial ? "block" : "none";
    document.body.classList.toggle("is-commercial", isCommercial);
    document.body.classList.toggle("is-public", !isCommercial);

    // Version is single-sourced from app/edition.py via the backend config.
    const appVersion = state.config?.version;
    if (appVersion) {
      document.querySelectorAll(".site-footer-version").forEach((el) => { el.textContent = appVersion; });
      const onboardingVersionEl = document.getElementById("onboarding-version");
      if (onboardingVersionEl) onboardingVersionEl.textContent = `Trade Hunter | Prediction Market Analysis Engine ${appVersion}`;
    }

    recordPriceHistory(state.activity || []);
    renderMetrics(state);
    renderAnalystAlert(state);
    ...
    const settingsPayload = {
        ...
        runtime_status: state.runtime_status || lastSettingsState?.runtime_status
    };
    renderSettings(settingsPayload);
...
if (tbFilterHideAppliedEl) {
  tbFilterHideAppliedEl.addEventListener("change", () => fetchAndRenderBacklog());
}
if (tbFilterHideRejectedEl) {
  tbFilterHideRejectedEl.addEventListener("change", () => fetchAndRenderBacklog());
}
...
function setServerActionProgress(message, tone = "pending") {
  setSettingsSaveStatus(message, tone);
}
...
if (settingsExitBtnEl) {
  settingsExitBtnEl.addEventListener("click", async () => {
    ...
    setServerActionProgress("Exit requested. Stopping Trade Hunter and closing local data files...", "pending");
    try {
      ...
      setServerActionProgress("Trade Hunter is shutting down. You can close this browser tab.", "success");
    } catch {
      setServerActionProgress("Trade Hunter is shutting down. You can close this browser tab.", "success");
    }
  });
...
if (settingsRestartBtnEl) {
  settingsRestartBtnEl.addEventListener("click", async () => {
    ...
    setServerActionProgress("Restart request accepted. Stopping server and closing the local database...", "pending");
    try {
      await fetch("/api/admin/shutdown", { 
        method: "POST", 
        headers: { "Content-Type": "application/json" }, 
      ...
      setServerActionProgress("Server is stopping. Waiting for dashboard health...", "pending");
    } catch {
      setServerActionProgress("Server connection closed. Waiting for dashboard health...", "pending");
    }
    ...
    const poll = setInterval(async () => {
      if (Date.now() - startedAt > RESTART_TIMEOUT_MS) {
        ...
        settingsRestartBtnEl.textContent = "Restart server";
        setServerActionButtonsDisabled(false);
        setServerActionProgress(
          "Restart is taking longer than expected. Close this tab, reopen Trade Hunter, or use Exit Trade Hunter if it is still running.",
          "error"
        );
        return;
      }
      ...
      try {
        ...
        if (r.ok) {
          clearInterval(poll);
          setServerActionProgress("Dashboard is ready. Reloading now...", "success");
          location.reload();
        } else {
          setServerActionProgress("Waiting for dashboard health...", "pending");
        }
...
if (adminTokenInputEl) {
  const isLoopback = ["127.0.0.1", "::1", "localhost"].includes(window.location.hostname);
...
function renderRulesetPanel() {
  if (!rulesetPanelEl || !rulesetSummaryEl || !rulesetCompactionEl || !rulesetPreviewEl || !rulesetLastRefreshEl) return;
  const panel = rulesetPanelEl;
  if (!panel) return;
  const hiddenByPage = panel.closest(".page-view")?.hidden ?? true;
  // panel lives in Settings, so only hit API when visible-ish
  if (panel.offsetParent === null) {
    rulesetSummaryEl.textContent = "idle";
    return;
  }
  ...
  const now = new Date();
  rulesetLastRefreshEl.textContent = "refreshing…";
  fetch("/api/ruleset", { method: "GET", headers })
    .then((r) => {
      if (!r.ok) throw new Error("ruleset request failed: " + r.status);
      return r.json();
    })
    .then((payload) => {
      const ruleset = payload?.ruleset || null;
      if (!ruleset) {
        rulesetSummaryEl.textContent = "Ruleset is disabled.";
        rulesetCompactionEl.textContent = "";
        rulesetPreviewEl.innerHTML = "-";
        return;
      }
      const counts = ruleset.counts || {};
      const total = counts.total || 0;
      const active = counts.active || 0;
      const retired = counts.retired || 0;
      const cap = counts.cap || 150;
      rulesetSummaryEl.textContent = "Active " + active + " / cap " + cap + " | total " + total + " | retired " + retired;
      const cc = ruleset.compaction_candidate_count || 0;
      const groups = ruleset.compaction_groups || [];
      if (cc) {
        rulesetCompactionEl.textContent = cc + " duplicate group(s) found · IDs: " + groups.slice(0, 10).flat().join(", ");
      } else {
        rulesetCompactionEl.textContent = "No duplicates found.";
      }
      const preview = ruleset.preview || [];
      if (!preview.length) {
        rulesetPreviewEl.innerHTML = '<span style="color:var(--text-muted);">No active rules yet — Apply tuning appends them here.</span>';
        return;
      }
      const rows = preview.map((r) => {
        const scope = ((r.scope || {}).topic || "all") + "/" + ((r.scope || {}).tier || "all");
        const source = r.source_tb ? " · " + escapeHtml(r.source_tb) : "";
        const meta = "hits:" + escapeHtml(String(r.hits || 0)) + " · " + escapeHtml(scope) + source;
        return '<div class="ruleset-row">'
          + '<span class="tuning-tb-id">' + escapeHtml(r.id) + '</span>'
          + '<span class="ruleset-row-text">' + escapeHtml(r.text) + '</span>'
          + '<span class="ruleset-row-meta">' + meta + '</span>'
          + '</div>';
      }).join("");
      rulesetPreviewEl.innerHTML = rows;
    })
    .catch((err) => {
      const msg = err && err.message ? err.message : "unknown error";
      rulesetSummaryEl.textContent = "Ruleset load failed: " + msg;
    })
    .finally(() => {
      rulesetLastRefreshEl.textContent = "last update " + now.toLocaleTimeString();
    });
}

function renderEvalPanel() {
  if (!evalStatusEl) return;
  // Only hit the API when the Settings page is actually visible.
  if (evalStatusEl.offsetParent === null) return;
  fetch("/api/eval/status", { method: "GET" })
    .then((r) => {
      if (!r.ok) throw new Error("eval status " + r.status);
      return r.json();
    })
    .then((s) => {
      if (!s || s.enabled === false) {
        evalStatusEl.innerHTML = '<span style="color:var(--text-muted);">Disabled — available in the Commercial Edition with an AI analyst configured.</span>';
        return;
      }
      const resolved = Number(s.resolved_scorable || 0);
      const pending = Number(s.pending || 0);
      const gates = s.gates || {};
      const firstGate = Number(gates.first_read || 30);
      const pubGate = Number(gates.publish || 100);
      const chain = s.chain_ok === false
        ? '<span style="color:var(--live,#f85525);font-weight:600;">chain BROKEN — do not score</span>'
        : 'chain OK';
      const toFirst = Math.max(0, firstGate - resolved);
      const toPub = Math.max(0, pubGate - resolved);
      // All values are numeric/boolean from the server — no user text interpolated.
      evalStatusEl.innerHTML =
        '<div class="eval-status-line">'
          + '<span>Resolved &amp; scorable: <strong>' + resolved + '</strong></span>'
          + '<span>pending: ' + pending + '</span>'
          + '<span>' + chain + '</span>'
        + '</div>'
        + '<div class="eval-status-sub">First read (' + firstGate + '): ' + (toFirst ? (toFirst + ' to go') : 'reached')
          + ' · Publish (' + pubGate + '): ' + (toPub ? (toPub + ' to go') : 'reached') + '</div>';
    })
    .catch(() => {
      evalStatusEl.innerHTML = '<span style="color:var(--text-muted);">Eval status unavailable.</span>';
    });
}
