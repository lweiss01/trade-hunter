...
Outputs:
    dist/trade-hunter-v0300-commercial.zip  — Gumroad upload (exe + source + README.txt)
    dist/trade-hunter-v0300-sim.exe         — Simulation exe (free / public)
...
def main() -> None:
    print("Trade Hunter Release Builder — v0.3.0")

    # Step 1: Commercial exe (built from master repo with IS_COMMERCIAL = True)
    run(
        "Step 1/3 — Commercial exe (trade-hunter-v0300.exe)",
        sys.executable, "-m", "PyInstaller",
        "trade-hunter-v0300.spec", "--distpath", "dist", "--noconfirm",
    )
    ...
    run(
        "Step 3/3 — Simulation exe (trade-hunter-v0300-sim.exe)",
        sys.executable, "-m", "PyInstaller",
        "trade-hunter-v0300-sim.spec", "--distpath", "dist", "--noconfirm",
    )
