...
def _read_master_version() -> str:
    """Parse VERSION from the master app/edition.py without importing the app.

    edition.py is the single source of truth for the version string; the public
    build's rewritten edition.py must carry it forward so nothing that reads
    VERSION breaks in the public/simulation edition.
    """
    edition_text = (ROOT / "app" / "edition.py").read_text(encoding="utf-8")
    match = re.search(r'^VERSION\s*=\s*["\'](.+?)["\']', edition_text, re.MULTILINE)
...
# Files that must survive the aggressive secret globs below. .env.example is a
# non-secret setup template that public users need; the `*.env*` glob would
# otherwise strip it from the distributed tree.
KEEP_NAMES = {n.lower() for n in {
    ".env.example",
}}
...
IGNORE_NAMES = {n.lower() for n in {
    ...
    ".tmp",
    ".artifacts",
    # Per-install runtime state (tuning ruleset lives in data/tuning; never distribute)
    "data",
...
def main():
    ...
    if edition_path.exists():
        ...
        print(f"[OK] IS_COMMERCIAL set to False, VERSION preserved as {version}.")
    else:
