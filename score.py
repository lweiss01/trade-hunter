...
Stdlib only. Run:  python score.py           (scores the app's live log)
             or:  python score.py <path>    (scores a specific log file)

The app writes its live log to data/eval/eval_log.jsonl — that is the file the
dashboard's Analyst Accuracy panel reads, and the default this script scores.
Do NOT point it at a stray eval_log.jsonl in the repo root; that is a stale
local artifact and will report far fewer pairs than the app actually has.
...
# The app's live eval log (same file the dashboard's Analyst Accuracy panel
# reads). Kept next to this script so the default works regardless of cwd.
DEFAULT_LOG = Path(__file__).resolve().parent / "data" / "eval" / "eval_log.jsonl"
...
def main():
    ...
    ap.add_argument("log", nargs="?", default=str(DEFAULT_LOG),
                    help=f"path to eval_log.jsonl (default: the app's live log at {DEFAULT_LOG})")
    ...
    if not Path(args.log).exists():
        raise SystemExit(
            ...
            f"The app's live log is {DEFAULT_LOG} — run the app to generate it, "
            f"or pass the correct path."
        )
    print(f"scoring: {args.log}")
