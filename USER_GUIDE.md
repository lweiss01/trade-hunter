...
**Last updated:** 2026-06-06 — v0.28.0
...
Double-click `trade-hunter-v0280.exe`. No Python, no terminal, no install.

**Setup — keep everything in one folder:**

```
Trade Hunter/
├── trade-hunter-v0280.exe
├── .env                      ← your settings and API keys
└── private_key.pem           ← your Kalshi private key
```

If a `.env` exists in the same folder as the exe, Trade Hunter uses that folder as its data root. This means your database, settings, and key file all stay together in one place you can find and back up easily.

Minimal `.env` for live mode:
```
ENABLE_KALSHI=true
...
If there is no `.env` next to the exe (first launch), Trade Hunter creates its data folder at `%LOCALAPPDATA%\Trade Hunter\` automatically.
...
| `signal` | ≥ 6.0 | Strong signal with multiple confirmations |
...
Trade Hunter enforces a single instance per port. Launching the exe while another instance is already running on the same port will ask the existing instance to shut down cleanly before the new one starts.

If you see alternating status strings in the feed detail (different `ws_msgs` counts on successive refreshes), a stray process may be holding the port. Kill all and restart clean:

```powershell
# Windows — kill any stray python instances, then relaunch exe
Get-CimInstance Win32_Process |
  Where-Object { $_.CommandLine -match "trade-hunter" -or $_.CommandLine -match "-m app" } |
  ForEach-Object { Stop-Process -Id $_.ProcessId -Force }
# Then double-click trade-hunter-v0280.exe
```
