...
- local dashboard with live signal feed, market tape, and feed health status
- SQLite-backed event and signal store with 7-day rolling retention
- spike detector using volume delta, price movement, rolling baselines, and directional coherence checks
- whale cluster detection via Poisson probability modeling (p < 0.01 threshold)
- 4-tier signal classification: `watch`, `notable`, `signal`, `whale-cluster`
- Kalshi live feed via `pykalshi` with full multi-tier ticker resolution (Series → Event → Market fan-out)
- category search for discovering active Kalshi markets without knowing ticker IDs
- dynamic ticker add/remove from the dashboard — no restart required
- PolyAlertHub-compatible ingest endpoint with Bearer auth
- dual AI signal analyst (Anthropic Claude primary, Perplexity fallback)
- autonomous tuning advisor + governor safety layer with durable backlog
- Discord webhook routing by topic with analyst-filtered alert modes
- simulation feed for local testing without API credentials
- commercial exe + simulation exe with automated build pipeline
- security hardening: admin token auth, CSRF protection, input validation

This is a working intelligence dashboard. Next priorities are cross-platform context and signal outcome tracking.
...
**Completed:**
- [x] **M014:** Essential Security Controls — Auth, CSRF, and injection hardening
- [x] **M015:** Ingest Integrity — Secure webhook authentication
- [x] **M016:** Security Awareness — Regression test suite and automation
- [x] **M017:** Distribution & Licensing — Public/Commercial edition separation, build pipeline
- [x] **M028:** Kalshi Multi-Tier Ticker Architecture — Series/Event/Market fan-out, hierarchy tracking, private key path resolution

**Up next:**
- ⬜ **M010:** Cross-Platform Whale Flow (Polymarket integration)
- ⬜ **M011:** Signal outcome tracking and feedback loops
- ⬜ **M012:** Cross-platform market matching and divergence monitoring
